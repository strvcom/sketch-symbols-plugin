...
export default class ElementTreeItem extends Component {
  ...
  renderQuickLook(expanded) {
    ...
    const { quickLook } = this.state
    return (
      <WrapElement
        onClick={e => {
          if (e.isDefaultPrevented() || element.id === '?') {
            return
          }
          this.setState({
            quickLook: !quickLook,
      ...
      >
        &lt;
        <CompleteElementName element={element} />
        {expanded ? '>' : ' />'}
        {quickLook && <QuickLook element={element} />}
      </WrapElement>
    )
  }
        ...
        <TreeElement expanded={expanded}>
          ...
          {expanded ? (
            <span>
              ...
              <WrapElement>
                &lt;/
                <CompleteElementName element={element} hideName />
                &gt;
              </WrapElement>
            </span>
          ) : (
